...
function findAntinodes(grid) {
  ...
  // find all antinodes on line in both directions
  const travelDownLine = (y, x, distY, distX) => {
    if (validateNodeLoc([y, x])) {
      if (!antiNodeCoords[`${y},${x}`]) {
        antiNodeCoords[`${y},${x}`] = true;
        numOfNodes += 1;
      }
      travelDownLine(y - distY, x - distX, distY, distX);
    }
  };

  const travelUpLine = (y, x, distY, distX) => {
    if (validateNodeLoc([y, x])) {
      if (!antiNodeCoords[`${y},${x}`]) {
        antiNodeCoords[`${y},${x}`] = true;
        numOfNodes += 1;
      }
      travelUpLine(y + distY, x + distX, distY, distX);
    }
  };
  ...
  const findAntinode = (y2, x2, y1, x1) => {
    ...
    if (validateNodeLoc([y2, x2]) && !antiNodeCoords[`${y2},${x2}`]) {
      antiNodeCoords[`${y2},${x2}`] = true;
      numOfNodes += 1;
    }

    if (validateNodeLoc([y1, x1]) && !antiNodeCoords[`${y1},${x1}`]) {
      antiNodeCoords[`${y1},${x1}`] = true;
      numOfNodes += 1;
    }

    travelDownLine(node2[0], node2[1], distY, distX);
    travelUpLine(node1[0], node1[1], distY, distX);
  };
